def test_finish_generates_report_and_get_report_returns_it(client):
    ...
    finish_response = client.post("/api/v1/interview/finish", json={"session_id": session_id})
...
def test_finish_rejects_session_that_is_not_ready(client):
    start_response = client.post(
        "/api/v1/interview/start",
        json={"prepare_id": "prepare-4", "mode": "standard", "planned_round_count": 3},
    )
    session_id = start_response.json()["session_id"]
...
def test_finish_and_report_reject_unknown_session(client):
    finish_response = client.post("/api/v1/interview/finish", json={"session_id": "missing-session"})
    report_response = client.get("/api/v1/report/missing-session")
